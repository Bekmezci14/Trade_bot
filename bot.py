...
class TradingBotGUI: 
    ...
    def add_equity(self):
        ...
        levels_prices = {i+1 : round(entry_price * (1 -drawdown * (i + 1)), 2) for i in range(levels)}
    ...
    def toggle_system(self):
        ...
        if not selected_items:
            messagebox.showwarning("Selection Error", "Please select an equity to toggle.")
            return
    ...
    def remove_selected_equity(self):
        selected_items = self.tree.selection()
        if not selected_items:
            messagebox.showwarning("Selection Error", "Please select an equity to remove.")
            return
        for item in selected_items:
            symbol = self.tree.item(item)['values'][0]
    ...
    def send_chat_message(self):
        ...
        if not message:
            return
        response = gemini.analyze_message(message)
    ...
    def check_existing_orders(self, symbol, price):
        try:
            orders = api.list_orders(status='open', symbol=symbol)
            for order in orders:
                if float(order.limit_price) == price:
                    return True
        except Exception as e:
            messagebox.showerror("API Error", f"Error checking existing orders: {e}")
        return False
        ...
        for symbol, data in self.equities.items():
            if data['status'] == "On":
                ...
                try:
                    ...
                    entry_price = self.get_max_entry_price(symbol)
                print(entry_price)
                levels_prices = {i+1: round(entry_price * (1 - data['drawdown'] * (i + 1)), 2) for i in range(len(data['levels']))}
                existing_levels = self.equities.get(symbol, {}).get('levels', {})
                for level, price in levels_prices.items():
    ...
    def place_order(self, symbol, price, level):
        if -level in self.equities[symbol]['levels'] or -1 in self.equities[symbol]['levels'].keys():
            return
        try:
            api.submit_order(symbol=symbol, qty=1, side='buy', type='limit', limit_price=price, time_in_force='gtc')
            self.equities[symbol]['levels'][-level] = price
            del self.equities[symbol]['levels'][level]
            print(f"Placed order for {symbol} at price {price}")
        except Exception as e:
